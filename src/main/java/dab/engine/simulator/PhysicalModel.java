...
public class PhysicalModel implements PlantController, PlantStatus {
    ...
    public Pump getPump(int pumpNumber) throws KeyNotFoundException {
        int actualIndex = pumpNumber - 1; // internally valves are stored from 0
        
        if (actualIndex < 0 || actualIndex >= allPumps.size())
            throw new KeyNotFoundException("Pump " + actualIndex + " does not exist");
    ...
    public Valve getValve(int valveNumber) throws KeyNotFoundException {
        ...
        if (actualIndex < 0 || actualIndex >= allConnections.size())
            throw new KeyNotFoundException("Valve " + actualIndex + " does not exist");
